module SystexJson.FSharpConverters.Tests
...
module Option =
    ...
    [<Test>]
    let ``Test option serializer, Some, record type`` () =
        IgnoreNulls.executeAsProperty (Some {TheVal = "hi"}) (Some """{"Value":{"TheVal":"hi"}}""")
        UseNulls.executeAsProperty (Some {TheVal = "hi"}) (Some """{"Value":{"TheVal":"hi"}}""")

    [<Test>]
    let ``Test option serializer, None, record type`` () =
        IgnoreNulls.executeAsProperty<RecordType option> None (Some """{}""")
        UseNulls.executeAsProperty<RecordType option> None (Some """{"Value":null}""")
